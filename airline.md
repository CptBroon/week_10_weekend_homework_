...
* an empty list of booked `people.Passenger`'s
* a `Plane`
...
* Add a 'flight' property to the `people.Passenger` which is assigned when a passenger is added to a flight
* Add a 'seat number' property to the `people.Passenger` as an integer. Set it to a random number which is assigned when a `people.Passenger` is booked on a flight
